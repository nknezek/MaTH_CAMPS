function T_out = convect_and_conduct(T, dt, Q_cmb, debug, debugerr)
...
r = pc.r;
N = pc.N;
E_i = core.utils.energy(T); % Initial Energy of Core
...
if max(dT_cond)>1
    if debugerr==1 || debug==1
        core.plot.temp_v_r(dT_cond, 'dTemperature')
        figure()
        hold on
    ...
    error('conduction is causing a temperature change > 1 K, use a smaller timestep. dt=%.2f Myr currently. error',dt/pc.Myr)
end
...
for i=N:-1:2
    T_im1a = core.utils.adiabat_from(T_regions(i),r(i),r(i-1));
    ...
    elseif T_im1a > T_regions(i-1)*epsilonTp1 % in conductive regime
        if in_an_adiabat == 1% in top layer of conductive regime
            ...
            for j=i:i_ad_top
                T_out(j) = core.utils.adiabat_from(T_ad_top_new, r(i_ad_top), r(j));
            end
            in_an_adiabat = 0;
            Ch_ad_total = 0;
