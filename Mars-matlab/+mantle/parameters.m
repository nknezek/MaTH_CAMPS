...
if layer_case == 0 % no layer
    ...
    eta_layer = 1e20; % [Pa-s] - viscosity lower layer 
...
elseif layer_case == 1 % hot start
    ...
    pm.rho=[3.2; 3.4; 3.888]*1e3; % density of layers, from mix run
...
pm.k(end) = 4.8;  % [W/m-K] - thermal conductivity for lower layer. 
pm.K = (1e-6)*ones_vec; % [m^2/s] - thermal diffusivity; 
pm.K(end) = 0.5e-6; % IMPORTANT: ONLY WORKS FOR 4-layer case

% Viscosity
pm.eta = 1e20*ones_vec;  % [Pa-s] - viscosity structure 
...
pm.Cp(end) = 1000; % [J/kg-K] heat capacity lower layer 
...
pm.H0 = partitions./pm.rho*8.16e-8; %heat production W/m^3 (input)
pm.lambda = ones_vec*1.37e-17;         % decay constants
