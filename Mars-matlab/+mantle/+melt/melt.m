...
for i=1:layer
    if (Ti(i) - Tliq(i) >= 0) && (Ti(i) - Tliq(i) >= 0) && (Ti(i)>Tprev(i)) % find Ti from solver above the liquidus
    ...
    if (Tt(i) <= Tliq(i)) && (Tt(i) >= Tsol(i)) % find Ti from solver above the solidus
        Tt(i) = (Ti(i)-Tprev(i))+ Tsol(i);                 % temperature value change over the solidus from preceding step - thermal history
        if Tt(i) > Tsol(i)
            f0(i) = (Tt(i)-Tsol(i))/(Tliq(i)-Tsol(i));          % melt fraction(not real)
            %Q0(i)=(Tt(i) - Tsol(i))*Cp(i)*M(i)*f0(i);         % energy with no phase changes (not real), fractional melt above solidus
            Q0(i)=(Tt(i) - Tsol(i))*Cp(i)*M(i);
            Q(i)=Q0(i)-M(i)*L(i)*f0(i);                              % System assumes Q0 is energy max, Q accounts for phase change (IS THIS CORRECT?)
            if Q(i) < 0                                          % if energy is negative, not enough T increase to melt entire layer
                f(i)=f(i)+Q0(i)/(M(i)*L(i));                       % Find f that will give Q=0
                Q1(i)=Q0(i)-M(i)*L(i)*f(i);
                if (Q1(i)~=0) && (Q1(i)/Q(i) < 1e-15)        % rounding errors in 16th decimal place common in matlab (what this looks like)
                    Q1(i)=0;                                     % sets likely rounding error to 0; Q < ~ 1e27, results in less than 1K change in T
                end
        ...
        if Q(i) > 0 && counts == 0
            ...
            dtm=Q(i)/(Cpm*M(i)*f(i));                          % excess energy to dt that goes into melt --  delQ=cpm*Mm*dtm of melt      %Q2(i)=Q(i)-cpm*Mm*dtm
            ...
            %                   if Q2 ~= 0
            %                       display ([num2str(Q2(i)) '  ' num2str(dtm) '  '  'Here there is a problem'])
            %                   else
            %                       display ([num2str(Q2(i)) '  ' num2str(dtm) '  '   'It works?'])
            %                   end
            %display ([num2str(Q(i)) ' ' num2str(Tt(i)-Tsol(i))])
            display ('Energy is greater than 0, can no longer neglect temperature change of parcel (e.g. include cp*M*dT + L*M)')
...
% for i=1:layer;
%     for j=2:length(Ti(:,i));
%         if (Tt(i) > Tsol(i)) && (f(i) == 0)
%             if count == 0
%                 Toff = (Tt(i)- Tsol(i));                                % determine initial offest - Note: will not work if systems warms again from this point
%             end
%             count = count+1;
%             if count > 0 & Toff >1
%                 %Tt(i) = ((Tt(i)- Tt(j-1,i)) + Tsol(i);
%                 Tt(i) = Tt(i) - Toff ;                                % fix remaining Temps to melt modified Temps
%             end
%         end
%     end
% end
