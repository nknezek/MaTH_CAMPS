...
for i=1:layer
    % shouldn't happen
%     if (Ti(i) >= Tliq(i)) && (Ti(i)>Tprev(i)) % find Ti from solver above the liquidus
%         disp("warning: Temperature is above liquidus, this shouldn't happen")
...
%     end
    % standard case 
    if (Ti(i) >= Tsol(i))             % find Ti from solver above the solidus
